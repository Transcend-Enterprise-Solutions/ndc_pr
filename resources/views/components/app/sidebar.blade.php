<div class="min-w-fit">
    ...
    <div id="sidebar"
        ...
        <div class="flex justify-left mb-10 ml-1" style="width: 200px; height: 32px;">
            <!-- Logo -->
            <a class="flex items-center mt-6" href="{{ route('dashboard') }}">
                <img class="mx-auto block dark:hidden" src="/images/logo.png" alt="beahoa logo" width="145">
                <img class="mx-auto hidden dark:block" src="/images/logo.png" alt="beahoa logo" width="145">
            </a>
        </div>
            ...
            <div>
                <ul class="mt-3">
                    ...
                    {{-- @if (Auth::user()->user_role === 'sa' || Auth::user()->user_role === 'admin') --}}
                        ...
                        <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-[linear-gradient(135deg,var(--tw-gradient-stops))] 
                        @if (in_array(Request::segment(1), ['budgets'])) {{ 'bg-gray-200 dark:bg-slate-900' }} @endif"
                            x-data="{ open: {{ in_array(Request::segment(1), ['budgets']) ? 1 : 0 }} }">
                            <a class="block text-gray-800 dark:text-gray-100 truncate transition 
                            @if (Route::is('budgets')) {{ '!text-blue-500' }} @endif"
                                href="{{ route('budgets') }}" wire:navigate>
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                        ...
                        <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-[linear-gradient(135deg,var(--tw-gradient-stops))] 
                        @if (in_array(Request::segment(1), ['procurement-planning'])) {{ 'bg-gray-200 dark:bg-slate-900' }} @endif"
                            x-data="{ open: {{ in_array(Request::segment(1), ['procurement-planning']) ? 1 : 0 }} }">
                            <a class="block text-gray-800 dark:text-gray-100 truncate transition 
                            @if (Route::is('procurement-planning')) {{ '!text-blue-500' }} @endif"
                                href="{{ route('procurement-planning') }}" wire:navigate>
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <i class="bi bi-diagram-3 text-slate-400 mr-3"></i>
                                        <span
